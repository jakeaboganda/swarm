...
- **`scenario`** — the scenario file schema (`ScenarioConfig`): arena
  ...
  pace). Each `AgentSlot` carries its `embodiment`, a list of `SensorDef`
  devices (`SensorSource::GroundTruth | Simulated` + a `SensorSpec` for
  simulated ones), and, for a `FmuVehicle`, its `fmu` binding. The reserved
  `GROUND_TRUTH_SENSOR` device name is always available to rules without being
  declared.
