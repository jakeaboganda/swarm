...
with -- and what it reports is range-limited, noisy and late.

`rule` validates its enums and raises ValueError on a bad one. A misspelt
action or operator would otherwise be a rule that quietly never fires.
"""
...
def rule(
    ...
    The general form -- the named constructors below cover the common cases.
    """
...
def brake_on_ttc(
    threshold: float,
    sensor: str = GROUND_TRUTH,
    priority: int = DEFAULT_PRIORITY,
) -> ReflexRule:
    """Brake while time-to-collision is under `threshold` seconds.

    Releases once the threat clears, so the car slows for traffic and carries
    on.
    """
...
def stop_on_ttc(
    ...
    """Stop dead and hold while time-to-collision is under `threshold` seconds.

    Use instead of `brake_on_ttc` when the car should park where it first saw
    trouble rather than creep forward as the reading oscillates.
    """
    return rule(sensor, ttc(), "less_than", threshold, "stop_and_hold", priority)


def stop_and_hold_above(
