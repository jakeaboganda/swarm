#!/usr/bin/env bash
# Reproducibly build opencardynamics_dt.fmu from source.
#
# Fetches Open-Car-Dynamics at a pinned commit + Eigen 3.4.0 into ./_work,
# builds the trimmed double-track FMI 3.0 CS wrapper, runs the proof harness,
# and assembles the .fmu. Idempotent. Linux/x86_64 only for now (a single
# binaries/x86_64-linux/opencardynamics_dt.so, like the VanDerPol fixture).
#
...
"${WORK}/build/harness" "${WORK}/build/opencardynamics_dt.so"

STAGE="${WORK}/fmu"
rm -rf "${STAGE}"
mkdir -p "${STAGE}/binaries/x86_64-linux"
cp "${HERE}/modelDescription.xml" "${STAGE}/"
cp "${WORK}/build/opencardynamics_dt.so" "${STAGE}/binaries/x86_64-linux/opencardynamics_dt.so"
( cd "${STAGE}" && zip -r -q "${HERE}/opencardynamics_dt.fmu" modelDescription.xml binaries )

echo "built ${HERE}/opencardynamics_dt.fmu"
